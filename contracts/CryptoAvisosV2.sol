...
contract CryptoAvisosV1 is Ownable {
    ...
    /* V2 variables */
    mapping(address => bool) private trustedSellers; //Mapping of trusted sellers. instead of removing trusted seller we can mark as false in bool value
    ...
    /// @notice Get all products filtered by `sellet`
    /// @param user address of user to filter
    /// @return an array of productsId
    function getProductsIdsByAddress(address user) public view returns (uint[] memory) {
        require(user != address(0), "!address");
        // Count how many of them are
        uint count = 0;
        for (uint256 i = 0; i < productsIds.length; i++) {
            if (productMapping[productsIds[i]].seller == user) {
                count++;
            }
        }
        ...
        // Add to array
        uint index = 0;
        uint[] memory _productsIds = new uint[](count);
        for (uint256 i = 0; i < productsIds.length; i++) {
            if (productMapping[productsIds[i]].seller == user) {
                _productsIds[index] = productsIds[i];
                index++;
            }
        }
        return _productsIds;
    ...
    function submitProduct(uint productId, address payable seller, uint price, address token, uint stock) external onlyTrusted {
        require(productId != 0, "!productId");
        require(price != 0, "!price");
        require(seller != address(0), "!seller");
        require(seller == msg.sender, "!productOwner");
    ...
    ///         Release pay can be done by owner (admin) or by product buyer
    /// @param ticketId TicketId (returned on `payProduct`)
    function releasePay(uint ticketId) external {
        Ticket memory ticket = productTicketsMapping[ticketId];
        require(owner() == msg.sender || ticket.buyer == msg.sender, "!access");
    ...
    /// @notice Used by product owner to modify your own product
    ...
    /// @param token address of the token
    /// @param stock how much units of the product
    function updateProduct(uint productId, address payable seller, uint price, address token, uint stock) external {
        //Update a product
        require(productId != 0, "!productId");
        require(price != 0, "!price");
        require(seller != address(0), "!seller");
        Product memory product = productMapping[productId];
        require(product.seller != address(0), "!exist");
        require(product.seller == msg.sender, "!productOwner");
    ...
    function addStock(uint productId, uint stockToAdd) external {
    ...
    function removeStock(uint productId, uint stockToRemove) external {
        //Add stock to a product
        Product memory product = productMapping[productId];
        require(productId != 0, "!productId");
    ...
    /**
     * @notice Throws if called by untrusted account.
     */
    modifier onlyTrusted() {
        require(trustedSellers[msg.sender] == true, "!trusted");
        _;
    }

    /// @notice Add address to trusted seller list
    /// @param _newTrustedSeller Seller address
    function addTrustedSeller(address _newTrustedSeller) external onlyOwner {
        require(_newTrustedSeller != address(0), "!address");
        
        trustedSellers[_newTrustedSeller] = true;
    }

    /// @notice Remove address from trusted seller list
    /// @param _trustedSeller Seller address
    function removeTrustedSeller(address _trustedSeller) external onlyOwner {
        require(_trustedSeller != address(0), "!address");
        
        trustedSellers[_trustedSeller] = false;

        // Disable seller products
        uint[] memory _productsIds = getProductsIdsByAddress(_trustedSeller);

        for (uint256 i = 0; i < _productsIds.length; i++) {
            Product memory product = productMapping[_productsIds[i]];
            product.enabled = false;
            productMapping[_productsIds[i]] = product;
                        
            emit SwitchChanged(_productsIds[i], false);
        }

    }

    /// @notice Check if address is a trusted seller
    /// @param _trustedSeller Seller address
    function checkTrustedSeller(address _trustedSeller) external view returns (bool) {
        require(_trustedSeller != address(0), "!address");
        
        return (true == trustedSellers[_trustedSeller]);
    }
