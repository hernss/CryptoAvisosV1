...
describe("CryptoAvisosV1", function () {
    ...
    it("Should add address to trusted seller list...", async function () {
        await this.cryptoAvisosV1.addTrustedSeller(seller.address);
        expect(await this.cryptoAvisosV1.checkTrustedSeller(seller.address)).to.be.true;

        await this.cryptoAvisosV1.removeTrustedSeller(seller.address);
        expect(await this.cryptoAvisosV1.checkTrustedSeller(seller.address)).to.be.false;
    });
    ...
    it("Should submit a product payable with DAI, succesfully...", async function () {
        ...
        //add seller to trusted list
        await this.cryptoAvisosV1.addTrustedSeller(seller.address);

        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(0, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!productId");
        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(productId, productSeller, ethers.utils.parseUnits("0", daiDecimals), productToken, stock)).to.be.revertedWith("!price");
        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(productId, ethers.constants.AddressZero, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!seller");
        //Test untrusted seller
        await expect(this.cryptoAvisosV1.connect(buyer).submitProduct(productId, buyer.address, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!trusted");
        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(productId, buyer.address, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!productOwner");
        await this.cryptoAvisosV1.connect(seller).submitProduct(productId, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock);
        ...
        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(productId, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("alreadyExist");
    ...
    it("Should submit a product payable with ETH, succesfully...", async function () {
        ...
        await this.cryptoAvisosV1.connect(seller).submitProduct(productId, productSeller, ethers.utils.parseUnits(productPrice), productToken, stock);
        ...
        await expect(this.cryptoAvisosV1.connect(seller).submitProduct(productId, productSeller, ethers.utils.parseUnits(productPrice), productToken, stock)).to.be.revertedWith("alreadyExist");
    ...
    it("Should update a product, successfully...", async function () {
        ...
        await expect(this.cryptoAvisosV1.connect(seller).updateProduct(0, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!productId");
        await expect(this.cryptoAvisosV1.connect(seller).updateProduct(productId, productSeller, ethers.utils.parseUnits("0", daiDecimals), productToken, stock)).to.be.revertedWith("!price");
        await expect(this.cryptoAvisosV1.connect(seller).updateProduct(productId, ethers.constants.AddressZero, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!seller");
        await expect(this.cryptoAvisosV1.connect(seller).updateProduct(8000, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!exist");
        await expect(this.cryptoAvisosV1.updateProduct(productId, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock)).to.be.revertedWith("!productOwner");

        await this.cryptoAvisosV1.connect(seller).updateProduct(productId, productSeller, ethers.utils.parseUnits(productPrice, daiDecimals), productToken, stock);
    ...
    it("Should release ETH from product pay...", async function () {
        ...
        await this.cryptoAvisosV1.connect(buyer).releasePay(ticketToRelease[0]);
        await expect(this.cryptoAvisosV1.connect(buyer).releasePay(ticketToRelease[0])).to.be.revertedWith("!waiting");
        ...
        expect(  Number(ethBalanceSellerAfter)).to.be.closeTo(Number(ethBalanceSellerBefore)   + Number(ethers.utils.formatUnits(product.price)) - (fee * Number(ethers.utils.formatUnits(product.price)) / 100), 0.000000001);
        expect(Number(ethBalanceContractAfter)).to.be.closeTo(Number(ethBalanceContractBefore) - Number(ethers.utils.formatUnits(product.price)) + (fee * Number(ethers.utils.formatUnits(product.price)) / 100), 0.000000001);
    ...
    it("Should add stock, successfully...", async function () {
        ...
        await this.cryptoAvisosV1.connect(seller).addStock(productToAdd, stockToAdd);
        await expect(this.cryptoAvisosV1.connect(seller).addStock(0, stockToAdd)).to.be.revertedWith("!productId");
        await expect(this.cryptoAvisosV1.connect(seller).addStock(productToAdd, 0)).to.be.revertedWith("!stockToAdd");
        await expect(this.cryptoAvisosV1.connect(seller).addStock(420, stockToAdd)).to.be.revertedWith("!exist");
        ...
        // other than product owner tried to add stock to product
        await expect(this.cryptoAvisosV1.connect(buyer).addStock(productToAdd, stockToAdd)).to.be.revertedWith("!productOwner");
    });
    ...
    it("Should remove stock, successfully...", async function () {
        ...
        // other than product owner tried to remove stock from product
        await expect(this.cryptoAvisosV1.connect(buyer).removeStock(productToRemove, stockToRemove)).to.be.revertedWith("!productOwner");

        await this.cryptoAvisosV1.connect(seller).removeStock(productToRemove, stockToRemove);
        await expect(this.cryptoAvisosV1.connect(seller).removeStock(0, stockToRemove)).to.be.revertedWith("!productId");
        await expect(this.cryptoAvisosV1.connect(seller).removeStock(productToRemove, 40)).to.be.revertedWith("!stockToRemove");
    ...
    it("Should getProductsIdsByAddress, successfully...", async function () {
        let productsIds = await this.cryptoAvisosV1.getProductsIdsByAddress(seller.address);
        expect(productsIds.length).greaterThan(0);
    });
